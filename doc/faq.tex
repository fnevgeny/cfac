...
The first paper that used results of FAC is Gu, M.F., 2003, ApJ, 582, 1241. The
code itself was described later in Gu, M.F., Can. J. Phys. 86: 675--689 (2008),
which can be used as the reference. A more detailed description of the code can
be found in the draft papers which are included in the \cFAC distribution.}
...
The function \key{OptimizeRadial} accepts a list of configurations, which form
the basis for the construction of the mean configuration. Usually, only the
lowest lying configurations should be used in \key{OptimizeRadial} for the
...
\key{SetCEQkMode} may be used to specify a different \key{QKMODE}.}
